...
NOTIFICATION_ICON="$HOME/.config/dunst/img/battery-level-10-symbolic.svg"
notified_20=false
notified_10=false
...
while true; do
    ...
    if [ "$BATTERY_LEVEL" -le "$LOW_BATTERY_THRESHOLD_10" ] && [ "$notified_10" = false ]; then
        notify-send -h string:bgcolor:#9e0000 -h string:fgcolor:#ffffff -h string:image-path:$NOTIFICATION_ICON "Critical Battery Level" "Battery level is at ${BATTERY_LEVEL}%"
        notified_10=true
    fi
